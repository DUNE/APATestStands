...
void GBiasFileReader(int startSerial, int endSerial) {
    ...
    TFile outFile("newGBiasTreeFile.root", "RECREATE");
    ...
    tree->Branch("Tolerance",        &Tolerance);
    ...
    for (int i = startSerial; i <= endSerial; i++) {
        std::string filename = "/home/tkdkrishna/BoardData/GBias/";
        ...
        while (getline(inputFile, line)) {
            ...
            getline(inputString, tempString, ',');
            Tolerance = std::atof(tempString.c_str());
            tempString = "";
